...
#        print(f"API 요청 실패: {response.status_code}, {response.text}")
    ...
    try:
        data = {
                ...
                "latitude": locations[closest][0],
                "longitude": locations[closest][1],
                "천안캠퍼스행": datetime.strftime(min([t for t in time_dts if t >= now], key=lambda x: abs(x -  now)), '%H:%M'),
                "아산캠퍼스행": datetime.strftime(min([t for t in time_dts_2 if t >= now], key=lambda x: abs(x - now)), '%H:%M')
        }
    except:
        data = {
                "station": closest,
                ...
                "latitude": locations[closest][0],
                "longitude": locations[closest][1],
                "천안캠퍼스행": datetime.strftime(time_dts[0], '%H:%M'),
                "아산캠퍼스행": datetime.strftime(time_dts_2[0], '%H:%M')
        }
